	...
	<div class='main clearfix'>
		...
		<div class='main_contain'>
			...
			<el-table :data="tableData" style="width: 100%;margin-top: 10px;" stripe border v-loading='loading'>
				...
				<el-table-column align="" label="项目" prop="fProjectName">
				...
				<el-table-column align="center" label="金额" prop="fAmount">
					<template slot-scope="scope">
	...
	export default {
		...
		methods: {
			...
			queryPage() {
				...
				this.axios.post('/perTaxToolTwo/e9/sheet/getCashFlowStatementPages', params)
			...
			exportExcel() {
				window.location.href = '/perTaxToolTwo/e9/sheet/exportCashFlow?fBrNo=' + this.fBrNo + '&fYear=' + this.fYear +
			...
			voucherGather_print() {
				...
				LODOP.PRINT_INIT("XJLLBDY"); // 现金流量表打印
				LODOP.SET_PRINT_PAGESIZE(1, 0, 0, "A4");
				LODOP.SET_PRINT_STYLE("FontSize", 11);
				LODOP.ADD_PRINT_TEXT(20, 300, 175, 30, "现金流量表");
				LODOP.SET_PRINT_STYLEA(0, "FontSize", 15);
				LODOP.SET_PRINT_STYLEA(0, "Bold", 1);
				//LODOP.ADD_PRINT_TEXT(50,25,150,20,"编制单位：");
				//var fBrNo = $("#vouchers_CustomerName_combobox").combobox('getText');
				LODOP.ADD_PRINT_TEXT(50, 25, 450, 20, "编制单位：" + this.fBrNo);
				var fYear = this.fYear;
				var fPeriod = this.fPeriod;

				var printDate = this.getCurrentPeriodDate(fYear, fPeriod);
				LODOP.ADD_PRINT_TEXT(50, 300, 175, 20, printDate);
				LODOP.ADD_PRINT_TEXT(50, 620, 70, 20, "单位：元");
				this.cashFlowSheet_lodopPrint(LODOP);
				LODOP.PREVIEW();
			},
			cashFlowSheet_lodopPrint(LODOP) {
				var var_table = "";
				var var_body = "";
				var var_head =
					"<table  border='1'  style='border-collapse:collapse;border-color: Black;border-bottom-color: Black;border-right-color: Black; font-size: 11pt; font-family: '宋体';'>";
				var_head += "<thead><tr ><td   style='width: 500px;' align='center'>项      目</td>" +
					"<td  style='width: 50px;'  align='center'>行次</td>" +
					"<td   style='width: 150px;'  align='center'>金额</td></tr></thead><tbody>";
				//
				...
				for (var i = 0; i < rows.length; i++) {
					...
					var_body += "<tr >";
					var_body += "<td style='width:500px;' align='left'>" + fProjectName + "</td>";
					var_body += "<td style='width:50px;' align='center'>" + fRowNumber + "</td>";
					var_body += "<td style='width:150px;' align='right'>" + fAmount + "</td>";
					var_body += "</tr>";

					if (fRowNumber == '56') {
						var_table = var_head + var_body + var_foot;
						LODOP.SET_PRINT_PAGESIZE(1, 0, 0, "A4");
						LODOP.ADD_PRINT_TABLE(80, 25, 650, 1050, var_table);
						var_body = '';
						continue;
					}
					if (fRowNumber == '83') {
