<template>
	<div class='main clearfix'>
		...
		<div class='main_contain'>
			...
			<el-table :data="tableData" style="width: 100%;margin-top: 10px;" stripe border v-loading='loading' v-if='customerType == 1'>
				...
				<el-table-column align="center" label="本月累计数" prop="fAmount1">
					<template slot-scope="scope">
						<span v-html='scope.row.fAmount1'></span>
					</template>
				</el-table-column>
				<el-table-column align="center" label="本年累计数" prop="fYtdAmount1">
					<template slot-scope="scope">
						<span v-html='scope.row.fYtdAmount1'></span>
					</template>
				</el-table-column>

			</el-table>
			
			<el-table :data="tableData" style="width: 100%;margin-top: 10px;" stripe border v-loading='loading' v-if='customerType == 2'>
				<!-- <el-table-column align="center" type="selection" width="55"></el-table-column> -->
				...
				<el-table-column align="center" label="本月非限定性" prop="monthUnrest1">
					<template slot-scope="scope">
						<span v-html='scope.row.monthUnrest1'></span>
					</template>
				</el-table-column>
				<el-table-column align="center" label="本月限定性" prop="monthRest1">
					<template slot-scope="scope">
						<span v-html='scope.row.monthRest1'></span>
					</template>
				</el-table-column>
				<el-table-column align="center" label="本月合计" prop="monthTotal1">
					<template slot-scope="scope">
						<span v-html='scope.row.monthTotal1'></span>
					</template>
				</el-table-column>
				<el-table-column align="center" label="本年非限定性" prop="yearUnrest1">
					<template slot-scope="scope">
						<span v-html='scope.row.yearUnrest1'></span>
					</template>
				</el-table-column>
				<el-table-column align="center" label="本年限定性" prop="yearRest1">
					<template slot-scope="scope">
						<span v-html='scope.row.yearRest1'></span>
					</template>
				</el-table-column>
				<el-table-column align="center" label="本年合计" prop="yearTotal1">
					<template slot-scope="scope">
						<span v-html='scope.row.yearTotal1'></span>
					</template>
				</el-table-column>
...
<script>
	export default {
		...
		methods: {
					...
					.then(res => {
						...
						if (res.data.code == 200) {
							...
							this.tableData.forEach(item => {
								item.fAmount1 = item.fAmount?item.fAmount.replace(/,/g,""):null;
								item.fYtdAmount1 = item.fYtdAmount?item.fYtdAmount.replace(/,/g,""):null;
								item.monthUnrest1 = item.monthUnrest?item.monthUnrest.replace(/,/g,""):null;
								item.monthRest1 = item.monthRest?item.monthRest.replace(/,/g,""):null;
								item.monthTotal1 = item.monthTotal?item.monthTotal.replace(/,/g,""):null;
								item.yearUnrest1 = item.yearUnrest?item.yearUnrest.replace(/,/g,""):null;
								item.yearRest1 = item.yearRest?item.yearRest.replace(/,/g,""):null;
								item.yearTotal1 = item.yearTotal?item.yearTotal.replace(/,/g,""):null;
							})
							this.$message({
			...
			/*利润表打印下载公共的方法----民非用户*/
			// 		profitAndLoss_lodopCommonMF(LODOP) {
			// 			var var_table = "";
			// 			var var_body = "";
			// 			var var_head =
			...
			// 			var_head += "<thead><tr ><td   style='width: 180px;' align='center'   height='28' valign='middle'>项名</td>" +
			// 				"<td style='width: 40px;'  align='center'   height='28' valign='middle'>行次</td>" +
			// 				"<td style='width: 70px;'  align='center'   height='28' valign='middle'>本月非限定性</td>" +
			// 				"<td style='width: 70px;'  align='center'   height='28' valign='middle'>本月限定性</td>" +
			// 				"<td style='width: 70px;'  align='center'   height='28' valign='middle'>本月合计</td>" +
			// 				"<td style='width: 70px;'  align='center'   height='28' valign='middle'>本年非限定性</td>" +
			// 				"<td style='width: 70px;'  align='center'   height='28' valign='middle'>本年限定性</td>" +
			// 				"<td style='width: 90px;'  align='center'   height='28' valign='middle'>本年合计</td></tr></thead><tbody>";
			// 
			// 			var var_foot = "</tbody></table>";
			// 			var rows = $('#profitAndLoss_dg').datagrid('getRows');
			// 			for (var i = 0; i < rows.length; i++) {
			// 				var row = rows[i];
			// 				var rowName = string_isNull(row.rowName);
			// 				var rowNum = string_isNull(row.rowNum);
			// 				var monthUnrest = string_isNull(row.monthUnrest);
			// 				var monthRest = string_isNull(row.monthRest);
			// 				var monthTotal = string_isNull(row.monthTotal);
			// 				var yearUnrest = string_isNull(row.yearUnrest);
			// 				var yearRest = string_isNull(row.yearRest);
			// 				var yearTotal = string_isNull(row.yearTotal);
			// 
			// 				var_body += "<tr >";
			// 				if (row.fRowNumber == "11") {
			// 					var_body += "<td style='width:220px;' align='left'   height='25' valign='middle'  style='font-size: 8pt;'>" +
			// 						rowName + "</td>";
			// 				} else if (rowName.length > 18) {
			// 					var_body += "<td style='width:220px;' align='left'   height='25' valign='middle'  style='font-size: 9pt;'>" +
			// 						rowName + "</td>";
			// 				} else if (rowName.length > 15) {
			// 					var_body += "<td style='width:220px;' align='left'   height='25' valign='middle'  style='font-size: 10pt;'>" +
			// 						rowName + "</td>";
			// 				} else {
			// 					var_body += "<td style='width:220px;' align='left'   height='25' valign='middle'  style='font-size: 9pt;'>" +
			// 						rowName + "</td>";
			// 				}
			// 				var_body += "<td style='width:50px;font-size: 10pt;' align='center'  height='25' valign='middle'>" + rowNum +
			// 					"</td>";
			// 				var_body += "<td style='width:60px;font-size: 10pt;' align='center'  height='25' valign='middle'>" + monthUnrest +
			// 					"</td>";
			// 				var_body += "<td style='width:60px;font-size: 10pt;' align='center'  height='25' valign='middle'>" + monthRest +
			// 					"</td>";
			// 				var_body += "<td style='width:60px;font-size: 10pt;' align='center'  height='25' valign='middle'>" + monthTotal +
			// 					"</td>";
			// 				var_body += "<td style='width:60px;font-size: 10pt;' align='center'  height='25' valign='middle'>" + yearUnrest +
			// 					"</td>";
			// 				var_body += "<td style='width:60px;font-size: 10pt;' align='center'  height='25' valign='middle'>" + yearRest +
			// 					"</td>";
			// 				var_body += "<td style='width:60px;font-size: 10pt;' align='center'  height='25' valign='middle'>" + yearTotal +
			// 					"</td>";
			// 				var_body += "</tr>";
			// 			}
