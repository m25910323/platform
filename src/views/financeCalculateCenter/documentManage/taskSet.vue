<template>
	<div class='main_contain'>
		<el-breadcrumb separator-class="el-icon-arrow-right">
			...
			<el-breadcrumb-item>盘库任务配置</el-breadcrumb-item>
		</el-breadcrumb>
		<div class="contain_body">
			...
			<el-table :data="tableList" v-loading='loading' @selection-change="handleSelectionChange">
				...
				<el-table-column align="center" label="盘点单识别号" prop="checkNo" :resizable="false"></el-table-column>
				...
				<el-table-column align="center" label="盘点人" prop="checkPerson" :resizable="false"></el-table-column>
				<el-table-column align="center" label="状态" :resizable="false">

					<template slot-scope="scope">
						<span v-show='scope.row.checkState == 0'>未开始</span>
						<span v-show='scope.row.checkState == 1'>盘点中</span>
						<span v-show='scope.row.checkState == 2'>已完成</span>
					</template>
				</el-table-column>
				
				<el-table-column align="center" label="制单人" prop="markerUserName" :resizable="false"></el-table-column>
				<el-table-column align="center" label="制单时间" prop="markDate" :resizable="false"></el-table-column>
				<el-table-column align="center" label="执行人" prop="performPersonName" :resizable="false"></el-table-column>
				...
				<el-table-column align="center" :resizable="false" width="100">
					...
					<template slot-scope="scope">
						<el-button size="mini" type="text" @click="handleJump(scope.$index, scope.row)">查看</el-button>
						<el-button size="mini" type="text" @click="handleEdit(scope.$index, scope.row,1)" v-show="scope.row.checkState == 0">开始</el-button>
						<el-button size="mini" type="text" @click="handleEdit(scope.$index, scope.row,2)" v-show="scope.row.checkState == 1">完成</el-button>
					</template>
				</el-table-column>
			</el-table>
	...
	export default {
		...
		methods: {
			queryListByPage() {
				...
				this.axios.post('/perTaxToolTwo/columbusNew/CheckMaterial/getCheckPrimaryListByPage', param).then(res => {
					this.loading = false;
			...
			handleJump(index, row){
				this.$router.push({
					path:'/financeCalculateCenter/detailSet',
					query:{
						checkId:row.checkId
					}
				})
			...
			handleEdit(index, row,tag){
				row.checkState = tag;
				let param = row;
				this.axios.post('/perTaxToolTwo/columbusNew/CheckMaterial/updateCheckPrimaryByPerformPerson', param).then(res => {
