	...
	<div class='main clearfix'>
			...
			<div>
				<el-form :model="uploadData" size="small" ref='formName' inline label-width="100px">
					<el-form-item label="用户名" prop="userName">
						<el-input v-model="uploadData.userName" placeholder="请输入" clearable filterable>
						</el-input>
					</el-form-item>

					<el-form-item label="有效开始时间" prop="keyValidityStartTime">
						<el-date-picker v-model="uploadData.keyValidityStartTime" type="date" placeholder="有效开始时间" value-format="yyyy-MM-dd"
						 format="yyyy-MM-dd">
						</el-date-picker>
					</el-form-item>
					<el-form-item label="有效结束时间" prop="keyValidityEndTime">
						<el-date-picker v-model="uploadData.keyValidityEndTime" type="date" placeholder="有效结束时间" value-format="yyyy-MM-dd"
						 format="yyyy-MM-dd">
						</el-date-picker>
					</el-form-item>
					<el-form-item label="Key状态" prop="keyStatus">
						<el-select v-model="uploadData.keyStatus" placeholder="请选择" filterable>
		...
		<div class='main_contain'>
			<div style="text-align: center;" class="clearfix">
				...
				<el-button class="el-icon-add" type='text' size="mini" @click='add()' style='float:right'>新增</el-button>
			</div>
			<el-table :data="tableData" style="width: 100%;" border v-loading='loading' @selection-change="handleSelectionChange10">
				...
				<el-table-column align="center" label="Key状态" :resizable="false">
					<template slot-scope="scope">
						<span v-if='scope.row.keyStatus == 0' style="color: #F56C6C;">无效</span>
						<span v-if='scope.row.keyStatus == 1' style="color: #67C23A;">有效</span>
					</template>
				</el-table-column>
				...
				<el-table-column align="center" label="操作" :resizable="false">
					<template slot-scope="scope">
						<el-button type="text" size="mini" @click='allocation(scope.row)'>权限分配</el-button>
						<el-button type="text" size="mini" @click='cancelAllocation(scope.row)'>取消分配</el-button>
		...
		<el-dialog title="新增" :visible.sync="dialogVisible" width="450px" :before-close="hideDialog">
			<el-form :inline="true" ref="ruleForm3" :model="info" class="demo-form-inline" label-width="120px" size="mini">
				...
				<div>
					<el-form-item label="有效开始时间" prop="keyValidityStartTime" id='file4'>
						<el-date-picker v-model="info.keyValidityStartTime" type="date" format="yyyy-MM-dd" value-format="yyyy-MM-dd HH:mm:ss"
						 placeholder="请选择"></el-date-picker>
					</el-form-item>
				</div>
				<div>
					<el-form-item label="有效结束时间" prop="keyValidityEndTime" id='file4'>
						<el-date-picker v-model="info.keyValidityEndTime" type="date" format="yyyy-MM-dd" value-format="yyyy-MM-dd HH:mm:ss"
						 placeholder="请选择"></el-date-picker>
					</el-form-item>
				</div>
			</el-form>
			<span slot="footer" class="dialog-footer">
				<el-button @click="hideClose">取 消</el-button>
		...
		<el-dialog title="修改" :visible.sync="dialogVisible2" width="450px" :before-close="hideDialog">
			<el-form :inline="true" ref="ruleForm3" :model="info" class="demo-form-inline" label-width="120px" size="mini">
				<div>
					<el-form-item label="用户名" prop="userName">
						<el-input v-model="info.userName" disabled></el-input>
				...
				<div>
					<el-form-item label="Key状态" prop="keyStatus">
						<el-select v-model="info.keyStatus" placeholder="请选择">
							<el-option v-for='item in keyStatusList' :label="item.label" :value='item.value'></el-option>
							<!-- <el-option label="无效" value='0'></el-option> -->
						</el-select>
					</el-form-item>
				</div>
	...
	export default {
		...
		methods: {
			...
			commitModify() {
				...
				let params = this.info;
				this.axios.post('/perTaxToolTwo/ArchivesPermissionInfo/updateUserIdentityInfo', params)
					// this.axios.post('/miaoxing/queryPage', params)
					.then(res => {
						if (res.data.code == 200) {
							this.queryPage();
			...
			handleDel(row) {
				this.$confirm('确认删除?', '提示', {
					...
					this.axios.post('/perTaxToolTwo/ArchivesPermissionInfo/deleteUserIdentityInfo', params)
						// this.axios.post('/miaoxing/queryPage', params)
						.then(res => {
							if (res.data.code == 200) {
								this.queryPage();
			...
			queryPage() {
				...
				let params = {
					...
					data: {
						...
						// "userName":"279845",
						//   "keyStatus": 1,
						//   "keyValidityStartTime": "2019-12-19",
						//   "keyValidityEndTime": "2019-12-31"
					}
