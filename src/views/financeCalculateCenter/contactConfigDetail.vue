<template>
	<div class='main clearfix'>
		<el-breadcrumb separator-class="el-icon-arrow-right">
			...
			<el-breadcrumb-item :to="{ path: '/financeCalculateCenter/contactConfig'}">往来信息配置</el-breadcrumb-item>
			<el-breadcrumb-item>详情</el-breadcrumb-item>
		</el-breadcrumb>
		<div class='search_contain'>
			<div>
				<el-form :model="uploadData" size="small" ref='formName' inline label-width="100px">
					<el-form-item label="月份" prop="customerId">
						<span>{{$route.query.year + '年' + $route.query.month}}</span>
					</el-form-item>
					<el-form-item label="每日最大承接客户数" prop="customerId" label-width="300px">
						<span>{{$route.query.maxUndertakeNumber}}家客户/天</span>
					</el-form-item>
					...
					<el-form-item label="公司类型" prop="isSenior">
						<el-select v-model="uploadData.isSenior" placeholder="请选择" clearable>
							<el-option v-for="item in typeList" :label="item.label" :value='item.value'></el-option>
						</el-select>
					</el-form-item>
					<el-form-item label="状态" prop="state">
						<el-select v-model="uploadData.state" placeholder="请选择" clearable>
		...
		<div class='main_contain'>
			<el-table :data="tableData" style="width: 100%;" border v-loading='loading'>
				...
				<el-table-column align="center" label="排序" prop="accountPeriod" :resizable="false"></el-table-column>
				<el-table-column align="center" label="预估处理日期" prop='preHandleTime' :resizable="false">
					<template slot-scope="scope">
						<span>{{scope.row.preHandleTime?formatTimeToStr(scope.row.preHandleTime,'yyyy-MM-dd'):'--'}}</span>
					</template>
				</el-table-column>
				<el-table-column align="center" label="公司名称" prop='customerName' :resizable="false"></el-table-column>
				<el-table-column align="center" label="公司类型" prop='isSenior' :resizable="false">
					<template slot-scope="scope">
						<span v-if="scope.row.isSenior == 1">中高端客户</span>
						<span v-else>普通客户</span>
					</template>
				</el-table-column>
				<el-table-column align="center" label="账期" prop='accountPeriod' :resizable="false"></el-table-column>
				<el-table-column align="center" label="问题数" prop='probNum' :resizable="false"></el-table-column>
				<el-table-column align="center" label="提交日期" prop='updateTime' :resizable="false"></el-table-column>
				<el-table-column align="center" label="状态" prop="state" :resizable="false">
					<template slot-scope="scope">
						...
						<span v-if="scope.row.state == 4">审核通过</span>
						<span v-if="scope.row.state == 5">确认无误</span>
						<span v-if="scope.row.state == 6">开始处理</span>
					</template>
				</el-table-column>
				<el-table-column align="center" label="操作" :resizable="false">
					<template slot-scope="scope">
						...
						<el-button type="text" size="mini" @click='handleQueueUp(scope.row)'>插队</el-button>
					</template>
				</el-table-column>
			</el-table>
		...
		<el-dialog width="1200px" title="详情" :visible.sync="dialogVisible">
			...
			<el-dialog width="1200" title="关联凭证" :visible.sync="dialogVisible3" append-to-body>
				...
				<div style="margin-bottom: 10px;">
					<label for="">账期：</label>
					<el-date-picker v-model="reaccountPeriod" type="month" placeholder="请选择" format="yyyy-MM" value-format="yyyy-MM"
					 disabled size="small" style="margin-left: 10px;"></el-date-picker>
				</div>
		...
		<el-dialog width="500px" title="插队" :visible.sync="dialogVisible4">
			<div style="margin-bottom: 10px;">
			...
			<div style="margin-bottom: 10px;">
				...
				<span>{{info.isSenior == 1?'中高端客户':'普通客户'}}</span>
			</div>
			<div style="margin-bottom: 10px;">
				<label for="">账期：</label>
				<span>{{info.accountPeriod}}</span>
			</div>
			<div slot="footer" class="dialog-footer">
				<el-button @click="dialogVisible4 = false">关闭</el-button>
				<el-button type="primary" @click="commitQueueUp">确定</el-button>
			</div>
		</el-dialog>
	...
	export default {
		...
		methods: {
			...
			handleDetail(row) {
				...
				this.updateDealConfirmRcrd(param,1);
				...
				this.axios.post('/perTaxToolTwo/e9/tVoucher/getProofTVoucher', this.qs.stringify(params), {
					...
					.then(res => {
						if (res.data.code == 200) {
							...
							this.reaccountPeriod = res.data.data.fyear.toString() + "-" + (res.data.data.fperiod < 10 ? '0' + res.data.data
			...
			queryPage() {
				...
				let params = {
					...
					data: {
						...
						"month": this.$route.query.month.slice(0,this.$route.query.month.length - 1),
						"state": this.state,
			...
			commitQueueUp(){
				...
				this.axios.post('/perTaxToolTwo/import/e9z/e9zBusinessDealConfirmRecordQueueUp', params)
					.then(res => {
						if (res.data.code == 200) {
			...
			updateDealConfirmRcrd(params,tag) {
		...
		watch: {
			...
			'$route'(to, from) {
				// init data
				if (to.name == 'contactConfigDetail') {
					this.queryPage();
				}
