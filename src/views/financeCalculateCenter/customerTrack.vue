<template>
	<div class="main_contain">
		<div class="left_contain">
			...
			<div class="contain_body">
				<el-table :data="banknameList" style="width: 100%" stripe border v-loading='loading'>
					...
					<el-table-column align="center" label="客户信息" prop="customerName" :resizable="false"></el-table-column>
					<el-table-column align="center" label="银行最后对账账期" prop="endPeriod" :resizable="false"></el-table-column>
					<el-table-column align="center" label="客户意愿" prop="customerState" :resizable="false">
						<template slot-scope="scope">
							<span v-show="scope.row.customerState == 1">有需求</span>
							<span v-show="scope.row.customerState == 2">没有需求</span>
							<span v-show="scope.row.customerState == 3">需求不明确</span>
						</template>
					</el-table-column>
		...
		<el-dialog title="查看" :visible.sync="dialogVisible" width="4rem" :before-close="handleClose">
	...
	export default {
		...
		methods: {
			getBankList() {
				...
				this.axios.post('/perTaxToolTwo/dz/customerTrackpool/queryListByPage', param).then(res => {
			...
			handleEdit(index, row) {
				...
				this.$prompt('请填写备注', '提示', {
					...
					this.axios.post('/perTaxToolTwo/dz/customerTrackpool/update',param).then(res => {
						if (res.data.code == 200) {
							this.getBankList();
							this.$message({
								message: res.data.msg,
			...
			compare() {
				let param = this.form;
				if (this.tag == 0) {
					var url = '/perTaxToolTwo/dz/dzDealController/saveBank';
				} else {
					var url = '/perTaxToolTwo/dz/dzDealController/updateBank';
				}
			...
			refresh() {
				...
				this.axios.post('/perTaxToolTwo/dz/dzDealController/getE9List', param).then(res => {
					if (res.data.code == 200) {
						...
						this.tbDzDetail.balance = res.data.data[0].balance;
					} else {
						this.$message({
							message: res.data.msg,
							type: 'error'
