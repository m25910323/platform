		...
		<div class="left_contain">
			<div class="contain_header">
				...
				<el-form :inline="true" :model="uploadData" class="demo-form-inline" size="mini">
					...
					<el-form-item>
						...
						<!-- <el-button @click='dzexport' size="mini">导出</el-button> -->
					</el-form-item>
				</el-form>
			</div>
			<div class="contain_body">
				<el-table :data="tableList" style="width: 100%" stripe border v-loading='loading'>
					...
					<el-table-column align="center" label="所有企业数" prop="cou" :resizable="false" width="300px">
						<template slot-scope="scope">
							<el-button type="text" size="mini">{{scope.row.cou}}</el-button>
						</template>
					</el-table-column>
					<el-table-column align="center" label="需求不明确" prop="unneed" :resizable="false">
						<template slot-scope="scope">
							<el-button type="text" size="mini" @click='showDetail(0,3)'>{{scope.row.unneed}}</el-button>
						</template>
					</el-table-column>
					<el-table-column align="center" label="无需求" prop="noneed" :resizable="false" width="300px">
						<template slot-scope="scope">
							<el-button type="text" size="mini" @click='showDetail(0,2)'>{{scope.row.noneed}}</el-button>
						</template>
					</el-table-column>
					<el-table-column align="center" label="有需求" prop="need" :resizable="false">
						<template slot-scope="scope">
							<el-button type="text" size="mini" @click='showDetail(0,1)'>{{scope.row.need}}</el-button>
						</template>
					</el-table-column>
					<el-table-column align="center" label="有需求且已提交"  prop="issubmit"   :resizable="false">
						<template slot-scope="scope">
							<el-button type="text" size="mini" @click='showDetail1(0)'>{{scope.row.issubmit}}</el-button>
						</template>
					</el-table-column>
				...
				<el-table :data="tableList1" style="width: 100%" stripe border v-loading='loading'>
					<!-- <el-table-column type="selection" width="50"></el-table-column> -->
				
					<el-table-column align="center" label="中高端总数" prop="cou" :resizable="false" width="300px">
						<template slot-scope="scope">
							<el-button type="text" size="mini">{{scope.row.cou}}</el-button>
						</template>
					</el-table-column>
					<el-table-column align="center" label="需求不明确" prop="unneed" :resizable="false">
						<template slot-scope="scope">
							<el-button type="text" size="mini" @click='showDetail(1,3)'>{{scope.row.unneed}}</el-button>
						</template>
					</el-table-column>
					<el-table-column align="center" label="无需求" prop="noneed" :resizable="false" width="300px">
						<template slot-scope="scope">
							<el-button type="text" size="mini" @click='showDetail(1,2)'>{{scope.row.noneed}}</el-button>
						</template>
					</el-table-column>
					<el-table-column align="center" label="有需求" prop="need" :resizable="false">
						<template slot-scope="scope">
							<el-button type="text" size="mini" @click='showDetail(1,1)'>{{scope.row.need}}</el-button>
						</template>
					</el-table-column>
					<el-table-column align="center" label="有需求且已提交"  prop="issubmit"   :resizable="false">
						<template slot-scope="scope">
							<el-button type="text" size="mini" @click='showDetail1(1)'>{{scope.row.issubmit}}</el-button>
						</template>
					</el-table-column>
					<el-table-column align="center" :resizable="false">
						<template slot="header" slot-scope="scope">
						...
						<template slot-scope="scope">
							<el-button size="mini" type="text" @click="dzexport(scope.row,1)">导出</el-button>
						</template>
					</el-table-column>
				</el-table>
			</div>
	...
	export default {
		...
		methods: {
			queryListByPage() {
				...
				this.axios.post('/perTaxToolTwo/dz/dzDealController/countAllCustomerNeed', param).then(res => {
					this.loading = false;
			...
			queryListByPage1() {
				...
				this.axios.post('/perTaxToolTwo/dz/dzDealController/countGdCustomerNeed', param).then(res => {
					this.loading = false;
					if (res.data.code == 200) {
			...
			queryDetail1(type){
				...
				this.axios.post('/perTaxToolTwo/dz/dzDealController/queryListCustomerByNeed', param).then(res => {
					this.loading = false;
					if (res.data.code == 200) {
						this.total = res.data.count;
						this.detail = res.data.data;
			...
			dzexport(row,tag){
				if(tag == 1){
					window.location.href = '/perTaxToolTwo/dz/dzDealController/downloadCustomerNeed?type=1';
				}else{
					window.location.href = '/perTaxToolTwo/dz/dzDealController/downloadCustomerNeed'
				}
