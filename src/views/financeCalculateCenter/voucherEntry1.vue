<template>
	<div class="main_contain">
		...
		<div class="contain_body">
			<el-table :data="detail" style="width:calc(100% - 40px);margin-top:20px;margin-bottom: 20px;" border v-loading='loading1' @row-click='showDetail' ref='refTable'
			...
			<div style="background-color: #fff;padding: 20px 50px 20px 20px;position: relative;">
				...
				<div class='line'></div>
				<div class='line'></div>
				<!-- <el-form :inline="true" :model="uploadData" class="demo-form-inline" size="mini" ref='formName' style="width: 800px;float: left;">
					<el-form-item label="单位名称" prop="fBrNo" :rules="[$rules.requiredSelect]">
						<el-select v-model="uploadData.fBrNo" placeholder="请选择单位名称" clearable filterable @change='search'>
				...
				<h4>
					...
					<span style="float: right;">凭证字号：记-{{voucherInfo.fnumber}}</span><span style="float: right;margin-right: 20px;">序列号：{{voucherInfo.fserialnum}}</span><span
					 style="float: right;margin-right: 20px;">{{uploadData.fYear.slice(0,4)}}年{{uploadData.fYear.slice(5,7)}}期</span>
				</h4>
				<el-table :data="tableList" style="width: 100%">
					<el-table-column align="center" label="摘要" width="450">
						<template slot-scope="scope">
							<div style="min-height: 20px;" :contenteditable="scope.$index != tableList.length - 1" v-text="scope.row.fexplanation" @blur='handleInput(scope.$index, "fexplanation",$event)'></div>
						</template>
					</el-table-column>
					<el-table-column align="center" prop="faccount" label="会计科目">
						<el-table-column align="center" label="一级科目_明细科目" prop="faccount" show-overflow-tooltip width="450">
							<template slot-scope="scope">
								<!-- <el-select v-show='scope.$index != tableList.length - 1 && tableList[0].fexplanation' size="mini" v-model="arr[scope.$index]"
								 placeholder="请选择" clearable filterable @change="((val,index)=>{handleSelect(val,scope.$index)})">
									<el-option v-for="item in fNameList" :label="item.fnumber + '-' + item.ffullname" :value='item.fnumber'></el-option>
								</el-select> -->
								<div style="min-height: 20px;" v-text="scope.row.faccountFull" v-show='scope.$index != tableList.length - 1 && tableList[0].fexplanation'
								 :contenteditable="scope.$index != tableList.length - 1 && tableList[0].fexplanation" @click="((index)=>{showDialog1(scope.$index)})">
								</div>
					...
					<el-table-column align="center" label="借方" prop="famount_string1">
						<template slot-scope="scope">
							<div style="min-height: 20px;" v-show="scope.$index != tableList.length - 1" :contenteditable="!scope.row.famount_string2" v-text="scope.row.famount_string1"
							 @blur='handleInput(scope.$index, "famount_string1",$event,"number")' @keydown.enter="cancelenter"></div>
					...
					<el-table-column align="center" label="贷方" prop="famount_string2">
						<template slot-scope="scope">
							<div style="min-height: 20px;" v-show="scope.$index != tableList.length - 1" :contenteditable="!scope.row.famount_string1" v-text="scope.row.famount_string2"
							 @blur='handleInput(scope.$index, "famount_string2",$event,"number")'></div>
				...
				<div class='btn_contain clearfix'>
					...
					<span class='close' @click="createProofTVouchers" :disabled="temList">提交</span>
				</div>
			</div>
		...
		<el-dialog title="查看" :visible.sync="dialogVisible1" width="1000px" class='dialog'>
			<div v-for='(item,index) in temList'>
				...
				<el-button type="primary" size="mini" style="margin-bottom: 10px;margin-left: 10px;" @click='deleteAll(index)' v-show='current == index'>删除全部</el-button>
		...
		<el-dialog width="400px" title="会计科目" :visible.sync="innerVisible1" append-to-body>
			<el-form ref="form" :model="form2" label-width="100px" size="mini">
				...
				<el-form-item label="科目类别">
					<el-select v-model='form2.fgroupid' clearable size="mini">
		...
		<el-dialog width="400px" title="会计科目" :visible.sync="innerVisible2" append-to-body>
			<el-form ref="form1" :model="form3" label-width="100px" size="mini">
				...
				<el-form-item label="科目类别">
					<el-select v-model='form3.fgroupid' clearable size="mini">
						<el-option v-for='item in fGroupList' :label="item.fGroupName" :value="item.fGroupID" :key='item.fGroupID'></el-option>
					</el-select>
				</el-form-item>
				<el-form-item label="余额方向">
					<el-select v-model='form3.fdc' clearable size="mini">
						<el-option v-for='item in fDCList' :label="item.fDCName" :value="item.fDC" :key='item.fDC'></el-option>
					</el-select>
				</el-form-item>
				<!-- <el-form-item label="损益所属科目">
	...
	export default {
		...
		methods: {
			deleteOneline(row,rowindex,index){
				this.$set(this.temList[index][rowindex],'fexplanation','');
				this.$set(this.temList[index][rowindex],'faccount','');
				this.$set(this.temList[index][rowindex],'famount_string1','');
				this.$set(this.temList[index][rowindex],'famount_string2','');
			},
			deleteOnePage(index){
			...
			renderContent(h, {
				...
				return ( <span style="flex: 1; display: flex; align-items: center; justify-content: space-between; font-size: 14px; padding-right: 8px;" >
					...
					commit() {
						...
						this.tableList[this.index]['faccount'] = this.$refs['tree' + this.tag].getCheckedNodes()[0].fnumber +
							'-' + this.$refs['tree' + this.tag].getCheckedNodes()[0].ffullname;
					...
					saveProofTVouchers() {
						...
						for (let i = 0; i < this.tableList.length - 1; i++) {
							tem.push(this.tableList[i]);
						}
						this.temList.push(tem);
						this.fdebittotal_stringList.push(this.famount_string1Total.replace(/,/g, ''));
						this.fcredittotal_stringList.push(this.famount_string2Total.replace(/,/g, ''));
						this.tableList = [{
								fexplanation: '',
								faccount: '',
								famount_string1: '',
						...
						// 			"faccount": item.fexplanation,
						// 			"fDC": 1,
						// 			"famount_string": item.famount_string1,
						// 		})
						// 	} else {
						// 		tvoucherentry.push({
						// 			"fexplanation": item.fexplanation,
						// 			"faccount": item.fexplanation,
						// 			"fDC": 0,
						// 			"famount_string": item.famount_string2,
						// 		})
						// 	}
						// })
						// let params = {
						// 	"fbrno": "新财务测试1",
						// 	"fdebittotal_string": this.famount_string1Total.replace(/,/g, ''),
						// 	"fcredittotal_string": this.famount_string2Total.replace(/,/g, ''),
						// 	"fexplanation": this.tableList[0].fexplanation,
						// 	"fentrycount": this.tableList.length - 1,
						// 	"tvoucherentry": tvoucherentry,
					...
					createProofTVouchers() {
						...
						// }else{
							...
							if(this.fcredittotal_string == this.fdebittotal_string){
								let params = {
									"fbrno": this.fBrNo,
									"fdebittotal_string": this.changeTwoDecimal(this.fdebittotal_string),
									"fcredittotal_string": this.changeTwoDecimal(this.fcredittotal_string),
									"fexplanation": tvoucherentry[0].fexplanation,
					...
					handleInput(index, label, event, tag) {
						if (tag != 'number') {

							this.tableList[index][label] = event.currentTarget.innerText;
						} else {
							// if(label == 'famount_string1')
							// console.log(1,event.currentTarget.innerText);
							// console.log(2,event.currentTarget.innerText.replace(/,/g, ''));
							// console.log(3,this.changeTwoDecimal(event.currentTarget.innerText.replace(/,/g, '')));
							// console.log(4,this.formatNum(this.changeTwoDecimal(event.currentTarget.innerText.replace(/,/g, ''))));
							if (isNaN(Number(event.currentTarget.innerText.replace(/,/g, '')))) {
