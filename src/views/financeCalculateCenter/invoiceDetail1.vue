	...
	<div class="main_contain">
		...
		<div class="left_contain">
			<div class="contain_header">
				...
				<el-button @click='handleJZ' round size='small' style="position: absolute;top: 100px;right: 20px;">反结账</el-button>
			</div>
				...
				<el-table :data="detail" style="width: 100%" stripe border v-loading='loading1'>
					...
					<el-table-column align="center" :resizable="false">
						...
						<template slot-scope="scope">
							...
							<el-button size="mini" type="text" @click="showJzDialog(scope.$index, scope.row)" v-show='scope.row.accountantTransferedCtStatus == 1 && scope.row.accountantIsCurrentPeriod == 1 && scope.row.accountantUncheckCount == 0 && scope.row.unDistributionCount == 0 && scope.row.unchecked == 0'>结账</el-button>
		...
		<el-dialog title="反结账" :visible.sync="dialogVisible2" width="400px" class='dialog' :before-close="handleClose">
			...
			<span slot="footer" class="dialog-footer">
				<el-button type="primary" @click="showJzDialog(1,formInline,1)">反结账</el-button>
				<!-- <el-button type="primary" @click="continueJZ" v-loading='loading11'>继续结账</el-button> -->
			</span>
			<!-- <div class='btn_contain clearfix'>
				<span class='close' @click="hideDialog" style='margin-right:10px;'>终止结账</span>
		...
		methods: {
			...
			showJzDialog(index, row, tag) {
				if (tag != 1) {
					...
					this.$confirm('<p>确认反结账？</p><p>' + row.accountPeriod.slice(0, 4) + '年' + row.accountPeriod.slice(5, 7) + '期</p>',
						'结账', {
							confirmButtonText: '确定',
							cancelButtonText: '取消',
							dangerouslyUseHTMLString: true
						...
						this.axios.post('/perTaxToolTwo/e9/tVoucherCheckout/createReverseVoucherCheckout', this.qs.stringify(params))
							// this.axios.post('/miaoxing/queryPage', params)
							.then(res => {
