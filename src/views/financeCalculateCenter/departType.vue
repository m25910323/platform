	...
	<div class='main clearfix'>
		...
		<div class='main_contain'>
			...
			<el-table :data="detail" style="width: 100%;margin-top: 12px;" stripe border>
				<el-table-column align="center" label="部门编号" prop="fnumber" :resizable="false"></el-table-column>
				<el-table-column align="center" label="部门名称" prop="fname" :resizable="false"></el-table-column>
		...
		methods: {
			...
			handleDel(index, row) {
				this.$confirm('确认删除', '提示', {
					...
					this.axios.post('/perTaxToolTwo/e9/TDepartment/deleteDepartment', params)
						// this.axios.post('/miaoxing/queryPage', params)
						.then(res => {
							if (res.data.code == 200) {
								this.getAssetGroups();
			...
			commitDialog(formName) {
				this.$refs['ruleForm'].validate((valid) => {
					if (valid) {
						...
						this.axios
							.post("/perTaxToolTwo/e9/TDepartment/saveDepartment", formData)
			...
			commitDialog1(formName) {
				this.$refs['ruleForm1'].validate((valid) => {
					if (valid) {
						...
						this.axios
							.post("/perTaxToolTwo/e9/TDepartment/updateDepartment", formData)
							.then(res => { //手动上传貌似无法触发成功或失败的钩子函数，因此这里手动调用 
								if (res.data.code == 200) {
									this.reset();
									this.getAssetGroups();
