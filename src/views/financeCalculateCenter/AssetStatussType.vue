<template>
	<div class='main clearfix'>
		<el-breadcrumb separator-class="el-icon-arrow-right">
			...
			<el-breadcrumb-item>固定资产使用状况</el-breadcrumb-item>
		</el-breadcrumb>
		...
		<div class='main_contain'>
			...
			<el-table :data="detail" style="width: 100%;margin-top: 12px;" stripe border>
				<el-table-column align="center" label="固定资产使用状况编号" prop="fnumber" :resizable="false"></el-table-column>
				<el-table-column align="center" label="固定资产使用状况名称" prop="fname" :resizable="false"></el-table-column>
		...
		<el-dialog title="新增" :visible.sync="dialogVisible" width="4rem" style="min-width:250px">
			<el-form :model="form" size="mini" label-width="100px" ref="ruleForm">
				...
				<el-form-item label="编号" prop="fnumber" :rules="[$rules.requiredInput]">
		...
		methods: {
			...
			handleDel(index, row) {
				this.$confirm('确认删除', '提示', {
					...
					this.axios.post('/perTaxToolTwo/e9/tfastatus/deleteAssetStatus', params)
						// this.axios.post('/miaoxing/queryPage', params)
						.then(res => {
							if (res.data.code == 200) {
								this.getAssetGroups();
			...
			commitDialog(formName) {
				this.$refs['ruleForm'].validate((valid) => {
					if (valid) {
						...
						this.axios
							.post("/perTaxToolTwo/e9/tfastatus/saveAssetStatus", formData)
			...
			commitDialog1(formName) {
				this.$refs['ruleForm1'].validate((valid) => {
					if (valid) {
						...
						this.axios
							.post("/perTaxToolTwo/e9/tfastatus/updateAssetStatus", formData)
							.then(res => { //手动上传貌似无法触发成功或失败的钩子函数，因此这里手动调用 
								if (res.data.code == 200) {
									this.reset();
									this.getAssetGroups();
