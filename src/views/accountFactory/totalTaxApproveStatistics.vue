<template>
	<div class='main clearfix'>
		<el-breadcrumb separator-class="el-icon-arrow-right">
			...
			<el-breadcrumb-item>税款审核统计表</el-breadcrumb-item>
		</el-breadcrumb>
	...
	export default {
		...
		methods: {
			...
			queryPage() {
				...
				this.axios.post('/perTaxToolTwo/e9z/assignment/countSksh', params)
					.then(res => {
						this.loading = false;
						if (res.data.code == 200) {
			...
			exportExcel(row, tag) {
				if (this.tag == 1) {
					window.location.href = '/perTaxToolTwo/e9z/assignment/exportTjb?accountantType=2' + '&accountPeriod=' + this.accountPeriod +
						'&filename=税款审核' + '&userName=' + this.row.user_name + '&finish=1'
				} else if (this.tag == 2) {
					window.location.href = '/perTaxToolTwo/e9z/assignment/exportTjb?accountantType=2' + '&accountPeriod=' + this.accountPeriod +
						'&filename=税款审核' + '&userName=' + this.row.user_name + '&finish=2'
				} else {
					window.location.href = '/perTaxToolTwo/e9z/assignment/exportTjb?accountantType=2' + '&accountPeriod=' + this.accountPeriod +
						'&filename=税款审核' + '&userName=' + this.row.user_name
				}
