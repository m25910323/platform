<template>
	<div class='main clearfix'>
		<el-breadcrumb separator-class="el-icon-arrow-right">
			...
			<el-breadcrumb-item>发票上传统计表</el-breadcrumb-item>
		</el-breadcrumb>
		...
		<div class='main_contain'>
			...
			<el-table :data="tableData" style="width: 100%;margin-top: 10px;" stripe border v-loading='loading'>
				...
				<el-table-column align="center" label="提交方式" prop="source"></el-table-column>
				<el-table-column align="center" label="未完成" prop='unsubmit'>
					<template slot-scope="scope">
						<!-- <el-button type="text" size="mini" :disabled="!scope.row.unsubmit" @click="exportExcel(scope.row,1)">{{scope.row.unsubmit}}</el-button> -->
						<el-button type="text" size="mini" :disabled="!scope.row.unsubmit" @click="showDetail(scope.row,1)">{{scope.row.unsubmit}}</el-button>
					</template>
				</el-table-column>
				<el-table-column align="center" label="已完成" prop="submit">
					<template slot-scope="scope">
						<el-button type="text" size="mini" :disabled="!scope.row.submit" @click="showDetail(scope.row,2)">{{scope.row.submit}}</el-button>
						
					</template>
				</el-table-column>
				<el-table-column align="center" label="合计" prop="total">
					<template slot-scope="scope">
						<el-button type="text" size="mini" :disabled="!scope.row.total" @click="showDetail(scope.row,'')">{{scope.row.total}}</el-button>
		...
		<el-dialog title="查看" :visible.sync="dialogVisible" width="1200px">
			...
			<el-table :data="detail.slice((currentPage - 1)*10,(currentPage - 1)*10 + 10)" style="width: 100%;margin-top: 10px;" stripe border v-loading='loading'>
				...
				<el-table-column align="center" label="发票张数" prop="cou"></el-table-column>
				<el-table-column align="center" label="发票审核人" prop="user_name"></el-table-column>
				<el-table-column align="center" label="APP确认状态" prop='appState'></el-table-column>
				<el-table-column align="center" label="最后一次提交人" prop="last_submit_user_name"></el-table-column>
				<el-table-column align="center" label="最后一次提交时间" prop="last_submit_time">
					<template slot-scope="scope">
	...
	export default {
		...
		methods: {
			...
			queryPage() {
				...
				this.axios.post('/perTaxToolTwo/iocr/main/getInvoiceStatisticsByAccountPeriod', params)
					.then(res => {
						this.loading = false;
						if (res.data.code == 200) {
			...
			exportExcel(row,tag) {
				if(this.row.source == '人工提交'){
					var sign = 2;
				}else if(this.row.source == 'APP提交'){
					var sign = 1;
				}
				if(this.tag == 1){
					window.location.href = '/perTaxToolTwo/e9z/assignment/exportTjb?sign=' + sign + '&finish=1' + '&filename=发票上传' + '&accountPeriod=' + this.accountPeriod
				}else if(this.tag == 2){
					window.location.href = '/perTaxToolTwo/e9z/assignment/exportTjb?sign=' + sign + '&finish=2' + '&filename=发票上传' + '&accountPeriod=' + this.accountPeriod
				}else{
					window.location.href = '/perTaxToolTwo/e9z/assignment/exportTjb?sign=' + sign + '&filename=发票上传' + '&accountPeriod=' + this.accountPeriod
				}
