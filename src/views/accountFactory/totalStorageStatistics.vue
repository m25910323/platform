<template>
	<div class='main clearfix'>
		<el-breadcrumb separator-class="el-icon-arrow-right">
			...
			<el-breadcrumb-item>凭证待入库统计表</el-breadcrumb-item>
		</el-breadcrumb>
		...
		<el-dialog title="查看" :visible.sync="dialogVisible" width="1200px">
			...
			<el-table :data="detail.slice((currentPage - 1)*10,(currentPage - 1)*10 + 10)" style="width: 100%;margin-top: 10px;" stripe border v-loading='loading'>
				...
				<el-table-column align="center" label="是否入库" prop="assign_time">
					<template slot-scope="scope">
	...
	export default {
		...
		methods: {
			...
			queryPage() {
				...
				this.axios.post('/perTaxToolTwo/e9z/assignment/countPzdrk', params)
					.then(res => {
						this.loading = false;
						if (res.data.code == 200) {
			...
			showDetail(row, tag) {
				...
				let params = {
					...
					"finish": this.tag,
					"accountantType":6,
					"userName":this.row.user_name !== '未分配'?this.row.user_name:''
				};
			...
			exportExcel(row, tag) {
				if (this.tag == 1) {
					window.location.href = '/perTaxToolTwo/e9z/assignment/exportTjb?accountantType=6' + '&accountPeriod=' + this.accountPeriod +
						'&filename=凭证待入库' + '&userName=' + this.row.user_name + '&finish=1'
				} else if (this.tag == 2) {
					window.location.href = '/perTaxToolTwo/e9z/assignment/exportTjb?accountantType=6' + '&accountPeriod=' + this.accountPeriod +
						'&filename=凭证待入库' + '&userName=' + this.row.user_name + '&finish=2'
				} else {
					window.location.href = '/perTaxToolTwo/e9z/assignment/exportTjb?accountantType=6' + '&accountPeriod=' + this.accountPeriod +
						'&filename=凭证待入库' + '&userName=' + this.row.user_name
				}
