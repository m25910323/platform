<template>
	<div class='main clearfix'>
		<el-breadcrumb separator-class="el-icon-arrow-right">
			...
			<el-breadcrumb-item>客户经理联系</el-breadcrumb-item>
		</el-breadcrumb>
			...
			<div>
				<el-form :inline="true" :model="uploadData" class="demo-form-inline" size="small" ref='formName'>
					...
					<el-form-item label="账期" prop="accountPeriod">
						<el-date-picker v-model="uploadData.accountPeriod" type="month" placeholder="请选择" value-format="yyyy-MM" format="yyyy-MM">
						</el-date-picker>
					</el-form-item>
		...
		<div class='main_contain'>
			...
			<el-table :data="tableData" style="width: 100%;margin-top: 10px;" border v-loading='loading'>
				...
				<el-table-column align="center" label="操作">
					<template slot-scope="scope">
						<el-button type="text" size="mini" @click='handleApprove(scope.row,2)'>详情</el-button>
						<!-- <el-button type="text" size="mini" @click='handleApprove1(scope.row,4)' v-show='scope.row.auditingStatus != 1'>流转</el-button> -->
						<el-button type="text" size="mini" @click='handleApprove1(scope.row,4)' v-show="scope.row.auditingStatus != 1">流转</el-button>
					</template>
				</el-table-column>
		...
		<el-dialog width="400px" title="客户经理联系" :visible.sync="dialogVisible">
			...
			<el-dialog width="400px" title="提示" :visible.sync="innerVisible" append-to-body>
				<p>该客户为中高端客户，或修改费用小于确认价格，是否确认开启审批流？</p>
				<el-form ref="form" :model="form1" label-width="140px" size="mini" label-position="left">
				
					<el-form-item label="修改类型">
						<el-input value="建议" disabled>
					...
					<el-form-item label="原值">
						<el-input type="textarea" v-model="form1.temrecommendInfo" disabled rows="10">
				
						</el-input>
				
					</el-form-item>
					<el-form-item label="修改值">
						<!-- <el-input v-model="form1.priceAccountManagerConfirmed" disabled>
				
						</el-input> -->
						
						<el-input type="textarea" v-model="form1.recommendInfo" disabled rows="10">
				...
				<span slot="footer" class="dialog-footer">
					...
					<el-button type="primary" @click="commit10">确 定</el-button>
				</span>
			</el-dialog>
		...
		<el-dialog title="详情" :visible.sync="dialogVisible1" width="400px">
			<el-form ref="form" :model="detail1" label-width="120px" size="mini">

				<el-form-item label="账务是否有问题">
...
<script>
	export default {
		...
		methods: {
			...
			commit() {
				if (this.form1.isSeniorCustomer == '1' || this.form1.tempriceAccountManagerConfirmed > this.form1.priceAccountManagerConfirmed) {
				...
				this.axios.post('/perTaxToolTwo/import/findYearCorpIncomeTaxById', params)
					.then(res => {
						if (res.data.code == 200) {
							...
							this.$set(this.form1, 'isSeniorCustomer', row.isSeniorCustomer);
				...
				this.axios.post('/perTaxToolTwo/e9z/businessAdjustmentExpensesInfo/get', params)
					.then(res => {
						if (res.data.code == 200) {
							...
							this.$set(this.form1, 'tempriceAccountManagerConfirmed', res.data.data.priceAccountManagerConfirmed);
							...
							this.$set(this.form1, 'temrecommendInfo', res.data.data.recommendInfo);
				...
				this.axios.post('/perTaxToolTwo/import/calcYearIncomeTax', params)
					...
					.then(res => {
						if (res.data.code == 200) {
							...
							this.$set(this.form1, "recommendInfo", "[" + JSON.parse(sessionStorage.getItem('updateUser')).nickName + "][" + this.formatTimeToStr(new Date(), 'yyyy-MM-dd hh:mm:ss') +
								"]" + ":客户尚有成本支出" + res.data.data.yearTax.advCostAmt +
								"元，费用支出" + res.data.data.yearTax.advFeeAmt + "元，需提供票据入账。重新核算后" + this.temYear + "年度汇算后需缴纳企业所得税" + res.data.data
								.yearTax.liquidationPayIncomeTax +
								"元（全年），要求票据在" + this.form1.timeRemindCustomerProvideInvoice + "前提供，账务整理费用" + this.form1.priceAccountManagerConfirmed +
								"元。本季度调整后利润总额:" + res.data.data.yearTax.qtrProfit +
			...
			commit10(){
				...
				var params = {};
				params.originalContent = this.form1.temrecommendInfo;
				params.updateContent = this.form1.recommendInfo;
				params.updateType = 2;
				params.adjExpId = this.form1.adjustmentExpensesInfoId;
				params.taxInfoId = this.form1.taxInfoId;
				this.axios.post('/perTaxToolTwo/e9z/e9zAuditingRecord/insertAuditingRecord', params)
					// this.axios.post('/miaoxing/queryPage', params)
					.then(res => {
						if (res.data.code == 200) {
							this.innerVisible = false;
