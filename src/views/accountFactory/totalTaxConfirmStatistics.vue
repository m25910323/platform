<template>
	<div class='main clearfix'>
		<el-breadcrumb separator-class="el-icon-arrow-right">
			...
			<el-breadcrumb-item>税款确认统计表</el-breadcrumb-item>
		</el-breadcrumb>
		...
		<div class='main_contain'>
			...
			<el-table :data="tableData" style="width: 100%;margin-top: 10px;" stripe border v-loading='loading' show-summary>
				...
				<el-table-column align="center" label="分配客服" prop="user_name">
		...
		<el-dialog title="查看" :visible.sync="dialogVisible" width="1200px">
			...
			<el-table :data="detail.slice((currentPage - 1)*10,(currentPage - 1)*10 + 10)" style="width: 100%;margin-top: 10px;" stripe border v-loading='loading'>
				...
				<el-table-column align="center" label="客服分配人" prop="skqr_user"></el-table-column>
				<el-table-column align="center" label="提交状态" prop='execute_status'></el-table-column>
				<el-table-column align="center" label="提交人" prop="execute_user_name"></el-table-column>
				<el-table-column align="center" label="提交时间" prop="assign_time">
					<template slot-scope="scope">
	...
	export default {
		...
		methods: {
			...
			queryPage() {
				...
				this.axios.post('/perTaxToolTwo/e9z/assignment/countSkqr', params)
					.then(res => {
						this.loading = false;
						if (res.data.code == 200) {
			...
			exportExcel(row, tag) {
				if (this.tag == 1) {
					window.location.href = '/perTaxToolTwo/e9z/assignment/exportTjb?accountantType=3' + '&accountPeriod=' + this.accountPeriod +
						'&filename=税款确认' + '&userName=' + this.row.user_name + '&finish=1'
				} else if (this.tag == 2) {
					window.location.href = '/perTaxToolTwo/e9z/assignment/exportTjb?accountantType=3' + '&accountPeriod=' + this.accountPeriod +
						'&filename=税款确认' + '&userName=' + this.row.user_name + '&finish=2'
				} else {
					window.location.href = '/perTaxToolTwo/e9z/assignment/exportTjb?accountantType=3' + '&accountPeriod=' + this.accountPeriod +
						'&filename=税款确认' + '&userName=' + this.row.user_name
				}
